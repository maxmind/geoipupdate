...
// CreateLockFile takes the provided filePath and lockFile name to create an flock.  All output errors are wrapped
//	in more detailed messages for debugging
func CreateLockFile(filePath, lockFile string, verbose bool) (*flock.Flock, error) {
	fi, err := os.Stat(filepath.Dir(filePath))
	if err != nil {
	...
	if !ok {
		return nil, errors.Errorf("could not acquire lock on %s", lockFile)
	}
	if verbose {
		log.Printf("Acquired lock file lock (%s)", lockFile)
	}
	return lock, nil
}
