...
struct hostent *GeoIP_get_host_or_proxy(void (*f) (char *))
{
    ...
    if (parse_http_proxy(&proxy_host, &proxy_creds, &proxy_port)) {
        ...
        encoded_proxy_creds_len =
            base64_encode_alloc(proxy_creds, strlen(proxy_creds),
                                &encoded_proxy_creds);
        if (encoded_proxy_creds == NULL) {
            if (encoded_proxy_creds_len == 0 && strlen(proxy_creds) != 0) {
                GeoIP_printf(f,
                             "Error processing proxy credentials: data too long: %d",
                             strlen(proxy_creds));
            } else {
                GeoIP_printf(f,
                             "Error processing proxy credentials: out of memory");
            }
        } else {
            GeoIPProxyCreds =
                malloc(sizeof(char) *
                       (strlen(ProxyAuthorization) +
                        strlen(encoded_proxy_creds) + 1));
...
short int GeoIP_update_database(char *license_key, int verbose,
                                void (*f) (char *))
{
    ...
    if ((cur_db_fh =
         fopen(GeoIPDBFileName[GEOIP_COUNTRY_EDITION], "rb")) == NULL) {
    ...
    request_uri =
        malloc(sizeof(char) * (strlen(license_key) + strlen(GeoIPHTTPRequest)
                               + strlen(GeoIPProxyHTTP) +
                               strlen(GeoIPProxiedHost) + 36 + 1));
    if (request_uri == NULL)
        return GEOIP_OUT_OF_MEMORY_ERR;
    sprintf(request_uri, GeoIPHTTPRequest, GeoIPProxyHTTP, GeoIPProxiedHost,
            license_key, hex_digest);
    ...
    } else if (strstr(compr, "Invalid product ID or subscription expired") !=
               NULL) {
        free(buf);
    ...
    file_path_gz =
        malloc(sizeof(char) *
               (strlen(GeoIPDBFileName[GEOIP_COUNTRY_EDITION]) + 4));
    ...
    file_path_test =
        malloc(sizeof(char) *
               (strlen(GeoIPDBFileName[GEOIP_COUNTRY_EDITION]) + 6));
...
short int GeoIP_update_database_general(char *user_id, char *license_key,
                                        char *data_base_type, int verbose,
                                        char **client_ipaddr,
                                        void (*f) (char *))
{
    struct hostent *hostlist;
    ...
    request_uri = malloc(sizeof(char) * (strlen(GeoIPHTTPRequestFilename)
                                         ...
                                         + strlen(data_base_type) +
                                         strlen(GeoIPUpdateHost) + 1));
    if (request_uri == NULL)
        return GEOIP_OUT_OF_MEMORY_ERR;

    /* get the file name from a web page using the product id */
    sprintf(request_uri, GeoIPHTTPRequestFilename, GeoIPProxyHTTP,
            GeoIPProxiedHost, data_base_type, GeoIPUpdateHost);
    if (verbose == 1) {
    ...
    if (client_ipaddr[0] == NULL) {
        ...
        sprintf(request_uri, GeoIPHTTPRequestClientIP, GeoIPProxyHTTP,
                GeoIPProxiedHost, GeoIPUpdateHost);
        GeoIP_send_request_uri(sock, request_uri);
        if (verbose == 1) {
    ...
    if (request_uri == NULL)
        return GEOIP_OUT_OF_MEMORY_ERR;
    snprintf(request_uri, request_uri_len, GeoIPHTTPRequestMD5, GeoIPProxyHTTP,
             GeoIPProxiedHost, hex_digest, hex_digest2, user_id,
             data_base_type);
    GeoIP_send_request_uri(sock, request_uri);
    if (verbose == 1) {
        GeoIP_printf(f, "sending request %s\n", request_uri);
